...
@Controller
public class AlarmMngController {  

    private String module_name = "alarm_mng";
    private CommonUtil common = new CommonUtil();
    
    @RequestMapping(value = "/alarm_mng")    
    // @ResponseBody    
    public ModelAndView alarm_mng(CommandMap body, HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        try {
            ...
            if(user!=null){
                ...
                String strSQL = "CALL SP_M_CMS_ALARM_LIST_1_1_65('"+SEARCH_TYPE+"','"+SEARCH_TXT+"','"+PAGE_SIZE+"','"+PAGE+"')";
                JSONArray jsonArray = common.execute_query(strSQL);
                view.addObject("data",jsonArray);

		        view.setViewName("alarm/alarm_mng");
            }
            else
            {
    ...
    @RequestMapping(value = "/alarm_mng/move/{search_type}/{search_txt}/{page_size}/{page}")    
    // @ResponseBody    
    public ModelAndView alarm_mng_move(CommandMap body, HttpServletRequest request,                                        
