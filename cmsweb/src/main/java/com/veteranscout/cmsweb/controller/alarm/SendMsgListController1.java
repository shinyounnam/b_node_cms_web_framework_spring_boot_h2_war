...
@Controller
public class SendMsgListController1 {  

    private String module_name = "send_msg_list1";
    private CommonUtil common = new CommonUtil();
    
    @RequestMapping(value = "/send_msg_list1/info/{corp_id}")    
    // @ResponseBody    
    public ModelAndView send_msg_list1_info(CommandMap body, HttpServletRequest request,
    ...
    ) 
    {
        ...
        try {
            ...
            if(user!=null){
                ...
                String strSQL = "";
                strSQL = "CALL SP_CMS_CORP_RECV_MSG_PAGE('"+CORP_ID+"','"+SEARCH_TYPE+"','"+SEARCH_TXT+"','"+PAGE+"')";
                JSONArray jsonArray = common.execute_query(strSQL);
                view.addObject("data",jsonArray);

		        view.setViewName("alarm/send_msg_list1");
            }
            else
            {
    ...
    @RequestMapping(value = "/send_msg_list1/move/{corp_id}/{search_type}/{search_txt}/{page}")    
    // @ResponseBody    
    public ModelAndView send_msg_list1_move(CommandMap body, HttpServletRequest request,
    @PathVariable("corp_id") int corp_id,
