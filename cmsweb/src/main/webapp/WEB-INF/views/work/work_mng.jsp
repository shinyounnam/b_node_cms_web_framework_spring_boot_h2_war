...
<div class="contents">
	...
	<div class="tab01">
		<ul>
			<li class="on"><a href="/work_mng">전체</a></li>
			<li><a href="/work_mng1">예약 현장</a></li>
			<li><a href="/work_mng2">진행 현장</a></li>
			<li><a href="/work_mng3">완료 현장</a></li><div class="font15 floatright bold">Today [<%=obj3_0.getString("TODAY_NM")%>]</div>
		</ul>
	</div>
	...
	<table class="list_A" summary="현장 현황 리스트">
		...
		<tr>
			<td><%=pageSeq%></td>
			<td>
				[<%=obj.getString("DATE_GBN_NM")%>]&nbsp;<%=obj.getString("SCHEDULE_NAME")%>
				<% if (obj.getString("S_HOUR").equals("")==false) { %>
					<br/>
					긴급 알림 발송 일시 <br/> 
					<%=obj.getString("S_START_DATE")%>~<%=obj.getString("S_END_DATE")%><br/>
					<%=obj.getString("S_HOUR")%>:<%=obj.getString("S_MIN")%>
				<% } %>
			</td>
			<td>
			...
			<td>
				...
				<% if (obj.getString("S_HOUR").equals("")==false) { %>
					<br/>
					<br/>
					<a href="javascript:OnWorkEdit('<%=obj.getInt("WORK_ID")%>')" class="btn01">수정</a>
				<% } %>
			</td>
		</tr>
		<%
...
<script type="text/javascript">
	...
	function OnWorkEdit(aWorkID)
	{
		var url = "";
		location.href="/v_req_schedule_add/update_info/"+aWorkID;
