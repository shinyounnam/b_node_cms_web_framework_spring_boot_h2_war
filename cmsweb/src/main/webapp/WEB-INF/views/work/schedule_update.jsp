...
<div class="contents">
	...
	<table class="list_A" summary="">
		...
		<tr>
			...
			<td colspan="4" style="text-align:left;">
				<input id="CORP_NAME" name="CORP_NAME" type="text" style="width:30%" value="<%=data0_0.getString("CORP_NAME")%>"/>
				<input type="hidden" id="CORP_ID" value="<%=data0_0.getInt("CORP_ID")%>"/>
				<input type="hidden" id="WORK_ID" value="<%=data0_0.getInt("WORK_ID")%>"/>
		...
		<tr>
			...
			<td style="text-align:left" colspan="4">
				<input id="WORK_DATE" name="WORK_DATE" type="text" style="width:20%" value="<%=data0_0.getString("WORK_DATE")%>"/>
				~
				<input id="TO_WORK_DATE" name="TO_WORK_DATE" type="text" style="width:20%" value="<%=data0_0.getString("TO_WORK_DATE")%>"/>
			</td>
		</tr>
		<tr>
			<th>현장명</th>
			<td colspan="4" style="text-align:left">
				<input type="text" id="NAME" name="NAME" style="width:80%" placeholder="현장 명칭을 입력해주세요" value="<%=data0_0.getString("NAME")%>"/>
			</td>
		</tr>
		<tr>
			<th rowspan="3">주소</th>
			<td colspan="4" style="text-align:left">
				<input type="text" id="POST_CODE" name="POST_CODE" style="width:30%" placeholder="우편번호" value="<%=data0_0.getString("POST_CODE")%>"/>
				<input type="text" id="CHAMGO" name="CHAMGO" style="width: 30%;" placeholder="참고항목" value="<%=data0_0.getString("CHAMGO")%>"/>
				<a href="#" onclick="SearchAddress();" class="btn01">주소검색</a>
			</td>
		</tr>
		<tr>
			<td colspan="4" style="text-align:left">
				<input type="text" id="ADDR1" name="ADDR1" style="width:80%" placeholder="도로명주소" value="<%=data0_0.getString("ADDR1")%>"/>
			</td>
		</tr>
		<tr>
			<td colspan="4" style="text-align:left">
				 <input type="text" id="ADDR2" name="ADDR2" style="width:80%" placeholder="상세주소" value="<%=data0_0.getString("ADDR2")%>"/>
				 <input type="hidden" id="POS_LAT" name="POS_LAT" value="<%=data0_0.getDouble("POS_LAT")%>"/>
				 <input type="hidden" id="POS_LNG" name="POS_LNG" value="<%=data0_0.getDouble("POS_LNG")%>"/>
				 <br/>
		...
		<tr>
			...
			<td style="text-align:left">
				<input type="text" id="MAN_NAME" name="MAN_NAME" style="width:90%" placeholder="담당자이름" value="<%=data0_0.getString("MAN_NAME")%>"/>
			</td>
			<th>직급</th>
			<td style="text-align:left">
				<input type="text" id="MAN_PSTN" name="MAN_PSTN" style="width:90%" placeholder="담당자 직급" value="<%=data0_0.getString("MAN_PSTN")%>"/>
			</td>
		</tr>
		<tr>
			<th>연락처</th>
			<td style="text-align:left">
				<input type="text" id="MAN_TEL" name="MAN_TEL" style="width:90%" placeholder="담당자 휴대폰" value="<%=data0_0.getString("MAN_TEL")%>"/>
			</td>
			<th>현장전화</th>
			<td style="text-align:left">
				 <input type="text" id="WORK_TEL" name="WORK_TEL" style="width:90%" placeholder="담당자 직통전화" value="<%=data0_0.getString("WORK_TEL")%>"/>
			</td>
		</tr>
		<tr>
			<th>*작업내용</th>
			<td style="text-align:left;" colspan="4">
				<textarea id="JOB_CONTENT" style="width:95%;height:250px;" name="JOB_CONTENT" row="20" placeholder="구제적인 작업내용을 작성해주세요."><%=data0_0.getString("JOB_CONTENT")%></textarea>
			</td>
		</tr>
	</table>
	<div class="pt30"></div>
	...
	<div class="pt30"></div>
	<%
		JSONArray list_data = new JSONArray(data0_0.getString("LIST_DATA"));
	%>
	<table id="WorkTable" class="list_A" summary="">
		...
		int index =0;
		int nIndex = 0;
		for(index=0;index<list_data.length();index++) {
            list_obj = list_data.getJSONObject(index);
			nIndex +=1;
		%>
		<tr>
			<td>
				<select id="JOB_GUBUN<%=nIndex%>" name="JOB_GUBUN" onchange="OnChangeJobGubun('<%=nIndex%>');" style="display: inline-block; width: 100px; margin-right: 10px;" class="form-control">
				  <%
					for (i=0;i<data1.length();i++) { 
                        
                        obj = data1.getJSONObject(i);
					    if (list_obj.getString("JOB_GBN") == obj.getString("CODE")) {
						%>
				...
				<select id="JOB_CODE<%=nIndex%>" name="JOB_CODE" style="display: inline-block; width: 130px;" class="form-control">
				  <%
					if(list_obj.getString("JOB_GBN").equals("1")) {

						for (i=0;i<data2.length();i++) { 

                            obj = data2.getJSONObject(i);
						    if (list_obj.getString("JOB_TYPE").equals(obj.getString("CODE"))) {
							%>
							    <option value="<%=obj.getString("CODE")%>" selected><%=obj.getString("NAME")%></option>
							<%
							}
					...
					else if(list_obj.getString("JOB_GBN").equals("2"))
					{
					...
					else if(list_obj.getString("JOB_GBN").equals("3"))
					{
			...
			<td>
				<input type="text" id="WORK_PAY<%=nIndex%>" name="WORK_PAY" value="<%=list_obj.getInt("WORK_PAY")%>" style="width: 75%; display: inline-block;" class="form-control">원 / 일
			</td>
			<td>
				<input type="text" id="WORK_CNT<%=nIndex%>" name="WORK_CNT" value="<%=list_obj.getInt("MAN_CNT")%>" style="width: 75%; display: inline-block;" class="form-control">명
			</td>
			<td>
				<% if(index != 0) { %>
				    <a id='DELETE_ROW<%=index%>' class='btn01' onclick='OnDeleteRow(<%=index%>);'>삭제</a>
				<% } %>
			</td>
		</tr>
		<%
	...
	<div class="pt30"></div>
	<div style="width:100%;border:0;text-align:right;">
		<a href="javascript:OnDelete();" class="btn03">삭제</a>
		<a href="javascript:OnSave();" class="btn03">저장</a>
		<a href="javascript:OnCancel();" class="btn04">취소</a>
		<input type="hidden" id="hidIndex" value="<%=nIndex%>"/>
	</div>
</div>
...
<script>
		...
		var coords1 = new daum.maps.LatLng(pos_lat, pos_lng);
				...
				geocoder.addr2coord(aADDR1, function(status, result) {
					...
					// 정상적으로 검색이 완료됐으면 
					 if (status === daum.maps.services.Status.OK) {
						...
						{
							if(markers[i]!=null) markers[i] = null;
						}
...
<script type='text/javascript'>
			...
			function OnSave()
			{
				...
				$.ajax({
		              		url: '/schedule_update/update_multi',
		              		type: 'POST',
