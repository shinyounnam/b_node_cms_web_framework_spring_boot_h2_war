...
<div class="contents">
	...
	<div class="tab01">
		<ul>
			<li class="on"><a href="javascript:OnMove('1');">보낸 메시지</a></li>
	...
	<table class="list_A" summary="직업소개소 알림">
		...
		<tr>
			<td><%=pageSeq%></td>
			<td><%=obj.getString("SEND_DATE")%></td>
			<td><%=obj.getString("RECV_NAMES")%></td>
			<td>
			<a href="javascript:OnMsgInfo('<%=obj.getString("BASE_TYPE")%>','<%=obj.getInt("CORP_ID")%>','<%=obj.getInt("MSG_ID")%>');">
			<%=obj.getString("CONTENT")%>
			</a>
			</td>
			<td><%=obj.getInt("READ_CNT")%>/<%=obj.getInt("TOTAL_CNT")%></td>
		</tr>
		<% 
...
<script type="text/javascript">
	...
	function OnPage(aPage)
	{
		...
		var PAGE = aPage;
		if (SEARCH_TXT=="") SEARCH_TXT = " ";
		location.href = "/send_msg_list/move" + 
				"/" + CORP_ID + 
				"/" + SEARCH_TYPE+
				"/"+SEARCH_TXT+
				"/"+PAGE+"/";
	...
	{
		var url = "/msg_info/info/"+aBaseType+"/"+aCorpID+"/"+aMsgID+"/";
		OpenWin(url,"msg_info",1000,600);
	}
</script>
